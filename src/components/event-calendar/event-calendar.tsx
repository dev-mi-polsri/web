...
// Update the EventCalendarProps interface to include a readOnly prop
...
// Update the EventCalendar component to use the readOnly prop
export function EventCalendar({
  ...
  const handleEventSelect = (event: CalendarEvent) => {
    ...
    // Only open the event dialog if not in read-only mode
    if (!readOnly) {
  ...
  const handleEventCreate = (startTime: Date) => {
    // Skip event creation in read-only mode
    if (readOnly) return
  ...
  const handleEventUpdate = (updatedEvent: CalendarEvent) => {
    // Skip event updates in read-only mode
    if (readOnly) return
  ...
  return (
    <div
      className="flex flex-col rounded-lg border has-data-[slot=month-view]:flex-1 w-full"
      style={
        {
